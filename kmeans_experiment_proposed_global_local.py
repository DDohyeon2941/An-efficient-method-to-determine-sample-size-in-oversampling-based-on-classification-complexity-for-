...
if __name__ == '__main__' :
    ...
    majority_label = 0
    k_value = 5
    c_names = np.array(['N1', 'N2', 'balance']).astype('str')

    for fname in dataset_fnames:
        start_time = time.time()
        temp_df = pd.read_csv(r'%s/%s'%(dataset_dir, fname))
        temp_complexity = pd.read_csv('resources\complexity\%s'%(fname))  # 파일 경로 직접 지정
        ...
        for rs_idx, rs in enumerate(random_states):
            ...
            for fidx, (train_index, test_index) in enumerate(kf.split(X, y)):
                ...
                complexity = temp_complexity.groupby(['dataset','rel_rs','rs','fold']).mean()
                fold_T = complexity.loc[fname.split('.')[0], rs_idx, rs, fidx][['N1', 'N2']].values
                ...
                for uni_ok_smote in smote_list:
                    for p_n, p_val in zip(c_names, fold_T):
                        T_value = lc.calculate_T_values(selected_cluster_list=uni_ok_smote.sel_cls,
                                                        metric_mode=p_n,
                                                        minority_class_indices=fold_trn_minidx,
                                                        neighbor_data=(N1_base_intra, N1_base_extra, N2_base))
           
                        base_size = int(fold_size_diff * p_val)
                        global_cls_size = np.around(uni_ok_smote.cls_sparsity * base_size)
                        local_cls_size = np.around(uni_ok_smote.cls_sparsity * T_value * fold_size_diff)
            
                        # checking valid 조건
                        if np.all(global_cls_size > 0) and np.all(local_cls_size > 0):
                            p_cls_case = ['global', 'local'] if T_value is not None else ['global']
            
                            for uni_case in p_cls_case:
                                learner = classifier()
                                if uni_case == 'global':
                                    ...
                                    learner.fit(X=X_train, y=y_train,
                                                n_samples=fold_size_diff, smote=uni_ok_smote,
                                                is_global=False, cls_T=T_value)
        ...
        result_df.to_csv(r'result/prediction_performance/proposed/kmeans/%s'%(fname), index=False)
